...
public class SUpdateInventoryMessage implements IModMessage {
    ...
    public enum Action {
        ITEMS, SELECTED, STATUS, ALL
    }

    public SUpdateInventoryMessage(@NotNull ModPlayerInventory inventory, Action action) {
    ...
    public SUpdateInventoryMessage(NonNullList<ItemStack> stacks, Action action, int selected, boolean status) {
        this.stacks = stacks;
        this.action = action;
    ...
    public static @NotNull SUpdateInventoryMessage decode(@NotNull PacketBuffer buffer) {
        return new SUpdateInventoryMessage(combine(buffer), buffer.readEnum(Action.class), buffer.readInt(), buffer.readBoolean());
    }
